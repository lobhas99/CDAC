...
jsp-api.jar --- <tomcat>/lib : specifications
...
1. param =Name of the map ,  created by WC :  containing request parameters 
eg : ${param.email}
=> out.print(request.getParameter("email"));
OR
out.print(param.get("email"));
2. pageScope=Name of the map ,  created by WC :  containing   page scoped attrs
eg : How to add a page scoped attribute ?
<%
  pageContext.setAttribute("num",1234);//auto boxing n upcasting
%>
How to access page scoped attr ?

${pageScope.num}


3. requestScope=map of request scoped attrs
4. sessionScope=map of session scoped attrs
5. applicationScope=map of application(=context) scoped attrs
6. pageContext --- instance of PageContext's sub class in jasper.jar
(Can be accessed via : scriptlet,JSP expression or EL syntax)

7. cookie -- map of cookies(cookie objects)
8. initParam -- map of context params.

eg : How will you send the value of a session scoped attr ("clnt_info") to clnt ?
${sessionScope.clnt_info}

${clnt_info}

How will you send the value of a  cookie by the name : JSESSIONID
${cookie.JSESSIONID.value}
=> out.print(cookie.get("JSESSIONID").getValue());
...
OR
<%= exception.getMessage() %>Additional EL syntax
