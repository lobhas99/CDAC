...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public String addEmpDetails(Employee emp) {
		...
		try {
			...
			tx.commit();/*
						 ...
						 * compares state of L1 cache with DB new entity : insert session.close() --L1
	...
	public Employee getEmpDetailsById(Integer empId) {
		...
		try {
			...
			tx.commit();// if not done : ERROR : connection leak
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
			throw e;
		}
		return emp;// emp : null | DETACHED from L1 cache
	}
	...
	public List<Employee> getEmsByDateAndSalary(LocalDate begin, LocalDate end1, double minSalary) {
		...
		Transaction tx = session.beginTransaction();
		try {
			emps = session.createQuery(jpql, Employee.class).setParameter("start", begin).setParameter("end", end1)
	...
	public List<Employee> testConstructorExpression(EmploymentType empType) {
		...
		Transaction tx = session.beginTransaction();
		try {
			emps = session.createQuery(jpql, Employee.class).setParameter("type", empType).getResultList();
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
	...
	public String updateEmpSalary(String email1, double increment) {
		...
		Transaction tx = session.beginTransaction();
		try {
			emp = session.createQuery(jpql, Employee.class).setParameter("em", email1).getSingleResult();// throws
																											// NoResultException
			// : in case // of no
			// result
			// found !
	...
	@Override
	public String storeImage(Integer empId, String fileName) throws IOException {
		String mesg = "Adding image failed !!!!";
		// 1. get session from SF
		Session session = getFactory().getCurrentSession();
		// 2. begin a tx
		Transaction tx = session.beginTransaction();
		try {
			...
			if (emp != null) {
				...
				if (file.isFile() && file.canRead()) {
					// => readable exisitng data file
					emp.setImage(FileUtils.readFileToByteArray(file));// modifying state of persistent entity
					mesg = "image stored....";
				}
			}

			tx.commit();// session.flush--> dirty chking --> update
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
			throw e;
	...
	@Override
	public String restoreImage(Integer empId, String fileName) throws IOException {
